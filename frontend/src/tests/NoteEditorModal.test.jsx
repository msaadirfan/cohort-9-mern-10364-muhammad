...
test("shows validation errors and blocks save when title and content are empty", async () => {
  ...
  try {
    ...
    expect(
      screen.getByText(/please enter a title/i)
    ).toBeInTheDocument();

    expect(
      screen.getByText(/please enter some content/i)
    ).toBeInTheDocument();

    expect(onSave).not.toHaveBeenCalled();
  } catch (error) {
    throw new Error("Failed to verify empty note validation", {
      cause: error,
    });
  }
});
...
test("calls onSave with trimmed title and HTML content when valid", async () => {
  ...
  try {
    await userEvent.type(
      screen.getByPlaceholderText(/note title/i),
      "  My Note  "
    );
    ...
    throw new Error("Failed to verify saving a valid note", {
      cause: error,
    });
  }
});